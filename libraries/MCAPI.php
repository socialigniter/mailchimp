...
class MCAPI
{
    ...
    var $errorMessage;
    var $errorCode;
    var $apiUrl; // Cache the information on the API location on the server
    var $timeout = 300; // Default to a 300 second timeout on server calls
    var $chunkSize = 8192;  // Default to a 8K chunk size
    var $api_key; // Cache the user api_key so we only have to log in once per client instantiation
    var $secure = false; // Cache the user api_key so we only have to log in once per client instantiation
    
    /* Connect to the MailChimp API for a given list.
     * @param string $apikey Your MailChimp apikey
     * @param string $secure Whether or not this should use a secure connection
     */
    function MCAPI($secure=false)
    {
        $this->secure	= $secure;
